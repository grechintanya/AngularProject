...
export const coursesReducer = createReducer(initialCoursesState,
    ...
    on(coursesActions.getCoursesSuccess, (state, { courses }): CoursesState =>
        ({ ...state, isLoading: false, courses, error: null })),
    on(coursesActions.getCoursesFailure, (state, { error }): CoursesState =>
        ({ ...state, isLoading: false, error })),
    on(coursesActions.searchQueryEntered, (state, { searchQuery }): CoursesState =>
        ({ ...state, searchQuery })),
    on(coursesActions.loadMoreSuccess, (state, { courses }): CoursesState =>
        ({ ...state, isLoading: false, courses: [...state.courses, ...courses], error: null })),
    ...
    on(coursesActions.deleteCourseSuccess, (state, { id }): CoursesState =>
        ({ ...state, isLoading: false, courses: [...state.courses.filter(item => item.id !== id)] })),
    ...
    on(coursesActions.createCourseSuccess, (state, { newCourse }): CoursesState =>
        ({ ...state, isLoading: false, error: null, courses: [...state.courses, newCourse] })),
    on(coursesActions.createCourseFailure, (state, { error }): CoursesState =>
        ({ ...state, isLoading: false, error })),
