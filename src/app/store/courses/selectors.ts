...
export const selectCoursesListLength = createSelector(selectCourses, (state) => state.courses.length);
export const selectSearchQuery = createSelector(selectCourses, (state) => state.searchQuery);
export const selectCourseById = (id: number) => createSelector(selectCourses,
    (state) => state.courses.find(item => item.id === id));
